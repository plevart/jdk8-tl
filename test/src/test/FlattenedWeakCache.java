...
public class FlattenedWeakCache<K, P, V> implements WeakCache<K, P, V> {
    ...
    public FlattenedWeakCache(BiFunction<? super K, ? super P, ?> subKeyFactory,
        ...
        this.reverseMap = supportContainsValueOperation ? new ConcurrentHashMap<>() : null;
    }
    ...
    /**
     * A factory {@link Supplier} that implements the lazy synchronized construction of the value and installment
     * of it into the cache.
     */
    private final class Factory implements Supplier<V> {
        ...
        public synchronized V get() { // serialize access
            ...
            if (supplier != this) {
                ...
                // return null to signal FlattenedWeakCache.get() to retry the loop
                return null;
            }
    ...
    /**
     * Common type of value suppliers that are holding a referent. The {@link #equals}
     * and {@link #hashCode} of implementations is defined to compare the referent by identity.
     */
    interface Value<V> extends Supplier<V> {}

    /**
     * An optimized {@link Value} used to look-up the value in {@link FlattenedWeakCache#containsValue} method
     * so that we are not constructing the whole {@link CacheValue} just to look-up the referent.
     */
    private static final class LookupValue<V> implements Value<V> {
        ...
        @Override
        public int hashCode() {
            return System.identityHashCode(value);        // compare referent by identity
        }
        ...
        public boolean equals(Object obj) {
            return obj == this ||
                   ...
                   this.value == ((Value<?>) obj).get();  // compare referent by identity
        }
    }

    /**
     * A {@link Value} that weakly references the referent and also holds a reference to {@code cacheKey}
     * so that it can implement {@link Expungable} by removing the corresponding entries from the maps.
     */
    private static final class CacheValue<V> extends WeakReference<V> implements Value<V>, Expungable {
        private final int hash;
        private final Object cacheKey;

        CacheValue(Object cacheKey, V value, ReferenceQueue<Object> refQueue) {
            super(value, refQueue);
            this.hash = System.identityHashCode(value); // compare referent by identity
            this.cacheKey = cacheKey;
        }
        ...
        public boolean equals(Object obj) {
            ...
            return obj == this ||
                   obj instanceof Value &&
                   (value = get()) != null &&        // cleared CacheValue is only equal to itself
                   value == ((Value<?>) obj).get();  // compare referent by identity
        }

        @Override
        public void expungeFrom(ConcurrentMap<?, ?> map, ConcurrentMap<?, Boolean> reverseMap) {
    ...
    private static final class CacheKey<K> extends WeakReference<K> implements Expungable {
        ...
        private CacheKey(K key, ReferenceQueue<Object> refQueue, Object subKey) {
            super(key, refQueue);
            this.hash = System.identityHashCode(key) * 31 +  // compare referent by identity
                        subKey.hashCode();
            this.subKey = subKey;
        }
        ...
        public boolean equals(Object obj) {
            ...
            return obj == this ||
                   ...
                   (thisKey = this.get()) != null &&  // cleared CacheKey is only equal to itself
                   thisKey == (other = (CacheKey<?>) obj).get() &&  // compare referent by identity
