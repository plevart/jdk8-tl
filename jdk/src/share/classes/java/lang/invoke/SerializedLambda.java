...
public final class SerializedLambda implements Serializable {
    ...
    private void writeObject(java.io.ObjectOutputStream out) throws IOException {
        ...
        // capturedArgs are transient to make sure they are last in stream
        out.writeObject(capturedArgs);
    }

    private transient Object lambdaOrProxy;

    private void readObject(java.io.ObjectInputStream in) throws IOException, ClassNotFoundException {
        // read all non-transient fields
        in.defaultReadObject();
        // capturedArgs are transient and last in stream.
        // if they contain a back reference to the lambda being de-serialized, readResolve() will
        // be called and it will return a proxy...
        capturedArgs = (Object[]) in.readObject();
        // resolve to lambda
        try {
            ...
            Object lambda = deserialize.invoke(null, this);
            if (lambdaOrProxy == null) {
                // readResolve has not been called during reading of capturedArgs
                // -> it will be after readObject() is finished
                lambdaOrProxy = lambda;
            } else {
                // read resolve has already been called during reading of capturedArgs and a proxy was created
                // -> install it's target lambda field...
                ((ProxyHandler) Proxy.getInvocationHandler(lambdaOrProxy)).lambda = lambda;
            }
        }
        catch (PrivilegedActionException e) {
            throw new RuntimeException("Exception in SerializedLambda.readObject", e.getCause());
        }
        catch (InvocationTargetException | IllegalAccessException e)
        {
            throw new RuntimeException("Exception in SerializedLambda.readObject", e);
        }
    }
    ...
    private Object readResolve() throws ReflectiveOperationException {
        if (lambdaOrProxy == null) { // create a proxy
            ClassLoader cl = capturingClass.getClassLoader();
            Class<?> funcInterface = Class.forName(functionalInterfaceClass, false, cl);
            return lambdaOrProxy = Proxy.newProxyInstance(cl, new Class<?>[] {funcInterface}, new ProxyHandler());
        } else { // just return already resolved lambda
            return lambdaOrProxy;
        }
    }
