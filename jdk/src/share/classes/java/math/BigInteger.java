...
public class BigInteger extends Number implements Comparable<BigInteger> {
    ...
    */
    ...
    public String toString(int radix) {
        ...
        if (signum < 0) {
            sb.append('-');
            return negate().nonNegativeToString(sb, radix, 0).toString();
        } else {
            return nonNegativeToString(sb, radix, 0).toString();
        }
    }

    /**
     * Converts this BigInteger to a string and appends to
     * <code>sb</code>.<p/>
     * This method is intended to be called for small
     * non-negative BigIntegers.
     *
    ...
    private StringBuilder smallNonNegaiveToString(StringBuilder sb, int radix, int digits) {
        ...
        String[] digitGroup;
        int numGroups;
        // Total number of digits without leading zeroes
        int len;

        if (signum == 0) {
            digitGroup = new String[] {"0"};
            numGroups = 1;
            len = 1;
        } else {
            // Compute upper bound on number of digit groups and allocate space
            int maxNumDigitGroups = (4*mag.length + 6)/7;
            digitGroup = new String[maxNumDigitGroups];
            ...
            len = (numGroups - 1)*digitsPerLong[radix] + digitGroup[numGroups-1].length();
        }

        // Fill with leading zeroes if needed
        if (len < digits) {
            // Prepend leading zeros
            int numLeadingZeros = digits - len;
            while (true) {
                if (numLeadingZeros < 64) {
                    sb.append(zeros[numLeadingZeros]);
                    break;
                } else {
                    sb.append(zeros[63]);
                    numLeadingZeros -= 63;
        ...
        sb.append(digitGroup[numGroups - 1]);
     ...
     * Answers to Exercises (4.4) Question 14.<p/>
     *
     * This method should only be called for positive BigIntegers.
     *
     * @param sb     The StringBuilder that will be appended to in place.
     * @param radix  The base to convert to.
     * @param digits The minimum number of digits to pad to.
     */
    private StringBuilder nonNegativeToString(StringBuilder sb, int radix, int digits) {
        ...
        results[0].nonNegativeToString(sb, radix, digits-expectedDigits);
        results[1].nonNegativeToString(sb, radix, expectedDigits);
